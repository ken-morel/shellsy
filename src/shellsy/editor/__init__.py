"""
eTextedit, from https://github.com/eliranwong/eTextEdit
                /blob/main/package/etextedit/etextedit.py
# eTextEdit

An Extensible Text Editor, built with prompt toolkit.

Originally a simple text editor created by Jonathan Slenders
Source: https://github.com/prompt-toolkit/python-prompt-toolkit/blob
                /master/examples/full-screen/text-editor.py
...
class TextInputDialog:
    def __init__(
        self, title="", label_text="", completer=None, validator=None
    ):
        self.future = Future()
...
class SearchReplaceDialog:
    def __init__(
        self, title="", label_find="", label_replace="", completer=None
    ):
        self.future = Future()
        ...
        def replace_all():
            self.future.set_result(
                (
                    True,
                    self.case_sensitivity.checked,
                    self.regex_search.checked,
                    self.search_area.text,
                    self.replace_area.text,
                )
            )
        ...
        self.search_area = TextArea(
            text=(
                ...
                else ApplicationState.search_pattern
            ),
            completer=completer,
            multiline=True,
            width=D(preferred=40),
        ...
        self.case_sensitivity = Checkbox(
            text="case-sensitive",
            checked=ApplicationState.enable_case_sensitive_search,
        )
        self.regex_search = Checkbox(
            text="regular expression",
            checked=ApplicationState.enable_regex_search,
        )

        replace_all_button = Button(text="All", handler=replace_all)
        selection_only_button = Button(
            text="Selection", handler=selection_only
        )
        cancel_button = Button(text="Cancel", handler=cancel)
        ...
        self.dialog = Dialog(
            ...
            body=HSplit(
                [
                    Label(text=label_find),
                    self.search_area,
                    Label(text=label_replace),
                    self.replace_area,
                    VSplit([self.case_sensitivity, self.regex_search]),
                ]
            ),
...
def _(event):
    ...
    if current_cursor_position_col >= text_field_width:
        ...
        previous_line = buffer.document.lines[
        ...
        if previous_line_last_chunk_width > current_cursor_position_col:
            buffer.cursor_position = (
                buffer.cursor_position - previous_line_last_chunk_width - 1
            )
        else:
            buffer.cursor_position = (
                buffer.cursor_position - current_cursor_position_col - 1
            )
...
def _(event):
    ...
    remaining_width_available = (
        text_field_width - current_chunk_cursor_position_col
    )
    if end_of_line_position > remaining_width_available:
        if end_of_line_position > text_field_width:
            ...
            buffer.cursor_position = (
                buffer.cursor_position + end_of_line_position
            )
    elif buffer.document.on_last_line:
        buffer.cursor_position = len(buffer.text)
    else:
        next_line = buffer.document.lines[
            buffer.document.cursor_position_row + 1
        ]
        ...
        if next_line_width >= current_chunk_cursor_position_col:
            buffer.cursor_position = (
                ...
                + current_chunk_cursor_position_col
                + 1
            )
        else:
            buffer.cursor_position = (
...
def _(event):
    buffer = event.app.current_buffer
    buffer.cursor_position = (
        buffer.cursor_position - buffer.document.cursor_position_col
    )
...
def _(event):
    ...
    search_state.text = ""
    text_field.buffer.apply_search(search_state, include_current_position=True)
...
def _(event):
    ...
    if previous_search_text:
        ...
        search_toolbar.search_buffer.cursor_position = len(
...
def do_save_file():
    if ApplicationState.current_path:
        try:
            with open(
                ApplicationState.current_path, "w", encoding="utf-8"
            ) as fileObj:
...
def do_find_replace():
    async def coroutine():
        ...
        if response is not None:
            ...
            original_text = (
                buffer.text if replace_all else buffer.copy_selection().text
            )
            replaced_text = re.sub(
                search_pattern if regex_search else re.escape(search_pattern),
                (
                    replace_pattern
                    if regex_search
                    else re.escape(replace_pattern)
                ),
                original_text,
...
def do_add_spaces(event=None):
    buffer = (
        event.app.current_buffer if event is not None else text_field.buffer
    )
    buffer.insert_text("    ")
...
def update_title():
    set_title(
        f"""eTextEdit - {os.path.basename(ApplicationState.current_path) if ApplicationState.current_path else "NEW"}"""
    )
